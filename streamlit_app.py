...
def load_data():
    ...
    # ⚡ Bolt Optimization: Pre-calculate derived UI state (options & maps)
    # inside cache to avoid O(N) recalculations on every Streamlit rerun.
    # Baseline ~0.018ms -> Optimized ~0.0004ms per load.
    indikaatiot = set()
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 1 & 2. Luodaan syöpätyypin valikko käyttäen esilaskettuja vaihtoehtoja
        valittu_syopatyyppi = st.selectbox("Syöpätyyppi", syopatyyppi_opts)
        
        # 3. Suodatetaan protokollat valitun syöpätyypin perusteella (O(1) haku)
