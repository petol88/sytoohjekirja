...
def load_data():
    ...
    for k in protokollat_map:
        protokollat_map[k].sort()

    return data, tuple(syopatyyppi_opts), protokollat_map

YKSIKKO_OPTS_BASE = ("mg/m2", "mg/kg", "AUC", "mg")

# Initialize global derived variables to prevent NameError if load fails
SYOPATYYPPI_OPTS = ("Kaikki",)
PROTOKOLLAT_MAP = {"Kaikki": []}

try:
    # Always update Tietokanta.data with cached/loaded data
    data, syopatyyppi_opts, protokollat_map = load_data()
    Tietokanta.data = data
    SYOPATYYPPI_OPTS = syopatyyppi_opts
    PROTOKOLLAT_MAP = protokollat_map
