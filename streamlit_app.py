...
def load_data():
    ...
    # Pre-calculate derived data for UI to avoid O(N) operations per Streamlit rerun
    indikaatiot = {t for p in data.values() for t in (p.get('syöpätyypit') or ["Ei määritelty"])}

    syopatyyppi_opts = tuple(["Kaikki"] + sorted(list(indikaatiot)))

    protokolla_map = { "Kaikki": tuple(data.keys()) }
    protokolla_map["Ei määritelty"] = tuple([
        nimi for nimi, d in data.items() if not d.get('syöpätyypit')
    ])
    for t in indikaatiot:
        if t == "Ei määritelty": continue
        protokolla_map[t] = tuple([
            nimi for nimi, d in data.items() if t in d.get('syöpätyypit', [])
        ])
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 2. Luodaan syöpätyypin valikko (optimoitu pre-calculated optiolla)
        valittu_syopatyyppi = st.selectbox("Syöpätyyppi", SYOPATYYPPI_OPTS)
        
        # 3. Suodatetaan protokollat valitun syöpätyypin perusteella (optimoitu O(1) lookupilla)
