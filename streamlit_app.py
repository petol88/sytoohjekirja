...
def load_data():
    ...
    # Pre-calculate unique cancer types
    indikaatiot = set()
    ...
    # Pre-calculate mapping of cancer type to protocols
    syopa_to_protokollat = {"Kaikki": tuple(data.keys())}
    syopa_to_protokollat["Ei määritelty"] = tuple([
        nimi for nimi, d in data.items()
        if not d.get('syöpätyypit')
    ])
    for tyyppi in indikaatiot:
        if tyyppi != "Ei määritelty":
            syopa_to_protokollat[tyyppi] = tuple([
                nimi for nimi, d in data.items()
                if tyyppi in d.get('syöpätyypit', [])
            ])

    return data, syopatyyppi_opts, syopa_to_protokollat
...
if view == "Laskuri":
    ...
    with col2:
        ...
        valittu_protokolla = st.selectbox("Protokolla", [""] + sorted(list(protokollat)))
