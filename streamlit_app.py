...
def load_data():
    ...
    # Pre-calculate derived data to avoid O(N) operations on every UI rerun
    indikaatiot = set()
    syopa_to_prot = {"Kaikki": list(data.keys()), "Ei määritelty": []}
    ...
    for nimi, prot_data in data.items():
        ...
        if tyypit:
            for t in tyypit:
                indikaatiot.add(t)
                if t not in syopa_to_prot:
                    syopa_to_prot[t] = []
                syopa_to_prot[t].append(nimi)
        else:
            indikaatiot.add("Ei määritelty")
            syopa_to_prot["Ei määritelty"].append(nimi)

    # Sort the lists for consistent UI
    indikaatiot_list = sorted(list(indikaatiot))
    for key in syopa_to_prot:
        syopa_to_prot[key] = sorted(syopa_to_prot[key])

    return data, indikaatiot_list, syopa_to_prot

YKSIKKO_OPTS_BASE = ("mg/m2", "mg/kg", "AUC", "mg")

# Initialize global fallback variables to avoid NameErrors if loading fails
cached_indikaatiot = []
cached_syopa_to_prot = {}

try:
    # Always update Tietokanta.data and derived mappings with cached/loaded data
    Tietokanta.data, cached_indikaatiot, cached_syopa_to_prot = load_data()
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 1. Käytetään valmiiksi laskettua syöpätyyppien listaa välimuistista
        opts = ["Kaikki"] + cached_indikaatiot
        valittu_syopatyyppi = st.selectbox("Syöpätyyppi", opts)
        
        # 2. Haetaan protokollat valmiiksi lasketusta sanakirjasta (O(1))
        protokollat = cached_syopa_to_prot.get(valittu_syopatyyppi, [])
            
        # 3. Protokollan valikko suodatetulla listalla
