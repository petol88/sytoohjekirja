...
def load_data():
    ...
    # Pre-calculate derived options and mappings to avoid O(N) operations on every UI rerun
    indikaatiot = set()
...
try:
    # Always update Tietokanta.data with cached/loaded data and load cached UI states
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 1. Luodaan syöpätyypin valikko (vaihtoehdot esilaskettu ja välimuistissa)
        valittu_syopatyyppi = st.selectbox("Syöpätyyppi", syopatyyppi_opts)
        
        # 2. Haetaan suodatetut protokollat suoraan esilasketusta sanakirjasta (O(1))
