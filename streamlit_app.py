...
def load_data():
    ...
    # Pre-calculate derived UI state to avoid O(N) recalculations on every Streamlit rerun
    indikaatiot = set()
    ...
    protokolla_map = {}
    for styyppi in syopatyyppi_opts:
        if styyppi == "Kaikki":
            protokollat = list(data.keys())
        elif styyppi == "Ei määritelty":
            protokollat = [nimi for nimi, pdata in data.items() if not pdata.get('syöpätyypit')]
        else:
            protokollat = [nimi for nimi, pdata in data.items() if styyppi in pdata.get('syöpätyypit', [])]
        protokolla_map[styyppi] = tuple(sorted(protokollat))
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 1. Luodaan syöpätyypin valikko (valmiiksi laskettu)
        ...
        valittu_protokolla = st.selectbox("Protokolla", [""] + list(protokollat))
