...
@st.cache_data
def get_protocol_mapping(data: dict) -> tuple:
    """Creates a cached mapping of cancer types to protocols for O(1) dropdown lookups."""
    indikaatiot = set()
    mapping = {"Kaikki": list(data.keys()), "Ei määritelty": []}

    for nimi, prot_data in data.items():
        tyypit = prot_data.get('syöpätyypit', [])
        if not tyypit:
            indikaatiot.add("Ei määritelty")
            mapping["Ei määritelty"].append(nimi)
        else:
            for t in tyypit:
                indikaatiot.add(t)
                if t not in mapping:
                    mapping[t] = []
                mapping[t].append(nimi)

    indikaatiot_list = ["Kaikki"] + sorted(list(indikaatiot))
    # Sort protocol lists internally
    for key in mapping:
        mapping[key] = sorted(mapping[key])

    return indikaatiot_list, mapping
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 1. Haetaan kertaalleen esilaskettu ja välimuistitettu lista sekä mappaus
        indikaatiot_list, protokollat_map = get_protocol_mapping(Tietokanta.data)
        
        # 2. Luodaan syöpätyypin valikko
        valittu_syopatyyppi = st.selectbox("Syöpätyyppi", indikaatiot_list)
        
        # 3. Haetaan protokollat O(1) valitun syöpätyypin perusteella
