...
def load_data():
    ...
    indikaatiot = set()
    protokollat_by_syopatyyppi = {"Kaikki": [], "Ei määritelty": []}

    for nimi, prot_data in Tietokanta.data.items():
        protokollat_by_syopatyyppi["Kaikki"].append(nimi)
    ...
    for k in protokollat_by_syopatyyppi:
        protokollat_by_syopatyyppi[k] = tuple([""] + sorted(protokollat_by_syopatyyppi[k]))

    return Tietokanta.data, syopatyyppi_opts, protokollat_by_syopatyyppi

YKSIKKO_OPTS_BASE = ("mg/m2", "mg/kg", "AUC", "mg")

SYOPATYYPPI_OPTS = ("Kaikki",)
PROTOKOLLAT_BY_SYOPATYYPPI = {}

try:
    # Always update Tietokanta.data with cached/loaded data
    data_tuple = load_data()
    Tietokanta.data = data_tuple[0]
    SYOPATYYPPI_OPTS = data_tuple[1]
    PROTOKOLLAT_BY_SYOPATYYPPI = data_tuple[2]
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 1. Luodaan syöpätyypin valikko
        valittu_syopatyyppi = st.selectbox("Syöpätyyppi", SYOPATYYPPI_OPTS)
        
        # 2. Suodatetaan protokollat valitun syöpätyypin perusteella
        protokollat = PROTOKOLLAT_BY_SYOPATYYPPI.get(valittu_syopatyyppi, ("",))
