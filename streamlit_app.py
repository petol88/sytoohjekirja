...
def load_data():
    ...
    # Pre-calculate derived UI mappings to avoid O(N) recalculations on every Streamlit rerun
    ...
    # Optimize for `in` checks and rendering speed by using tuples instead of lists
    syopatyyppi_opts_tup = tuple(["Kaikki"] + sorted(list(syopatyyppi_opts)))
    for k in protokolla_map:
        protokolla_map[k] = tuple(sorted(protokolla_map[k]))

    return Tietokanta.data, syopatyyppi_opts_tup, protokolla_map
