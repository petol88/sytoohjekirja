...
def load_data():
    ...
    syopatyyppi_lista = ["Kaikki"] + sorted(list(indikaatiot))

    return Tietokanta.data, tuple(syopatyyppi_lista), protokollat_by_syopatyyppi

YKSIKKO_OPTS_BASE = ("mg/m2", "mg/kg", "AUC", "mg")

try:
    # Always update Tietokanta.data with cached/loaded data
    _data, _syopatyyppi_lista, _protokollat_by_syopatyyppi = load_data()
    Tietokanta.data = _data
except Exception as e:
    st.error(f"Virhe ladattaessa tietokantaa: {e}")
    _syopatyyppi_lista = ("Kaikki",)
    _protokollat_by_syopatyyppi = {"Kaikki": []}
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 2. Luodaan syöpätyypin valikko esilasketusta listasta
        valittu_syopatyyppi = st.selectbox("Syöpätyyppi", _syopatyyppi_lista)
        
        # 3. Haetaan protokollat valitun syöpätyypin perusteella
        protokollat = _protokollat_by_syopatyyppi.get(valittu_syopatyyppi, [])
