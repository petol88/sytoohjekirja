...
def load_data():
    ...
    indikaatiot = set()
    syopa_to_protokollat = {}

    for nimi, prot_data in data.items():
        tyypit = prot_data.get('syöpätyypit', [])
        if not tyypit:
            indikaatiot.add("Ei määritelty")
            syopa_to_protokollat.setdefault("Ei määritelty", []).append(nimi)
        else:
            for t in tyypit:
                indikaatiot.add(t)
                syopa_to_protokollat.setdefault(t, []).append(nimi)

    for k in syopa_to_protokollat:
        syopa_to_protokollat[k] = sorted(syopa_to_protokollat[k])

    syopa_to_protokollat["Kaikki"] = sorted(list(data.keys()))

    indikaatio_opts = tuple(["Kaikki"] + sorted(list(indikaatiot)))
    return data, indikaatio_opts, syopa_to_protokollat

YKSIKKO_OPTS_BASE = ("mg/m2", "mg/kg", "AUC", "mg")

try:
    # Always update Tietokanta.data with cached/loaded data
    _data, SYOPA_OPTS, SYOPA_MAPPING = load_data()
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 1-2. Valikko esilasketuilla vaihtoehdoilla
        valittu_syopatyyppi = st.selectbox("Syöpätyyppi", SYOPA_OPTS)
        
        # 3. Haetaan protokollat valitun syöpätyypin perusteella
        protokollat = SYOPA_MAPPING.get(valittu_syopatyyppi, [])
