...
def load_data():
    ...
    for nimi, prot_data in Tietokanta.data.items():
        ...
        # Oletuksena tyhjä lista jos ei löydy
        if tyypit:
            for t in tyypit:
                indikaatiot.add(t)
        ...
        # Lisätään kaikki protokollat "Kaikki" kategoriaan
        if "Kaikki" not in protokolla_map:
            protokolla_map["Kaikki"] = []
        protokolla_map["Kaikki"].append(nimi)

    syopatyyppi_opts = tuple(["Kaikki"] + sorted(list(indikaatiot)))

    # Järjestetään protokollalistat
    for t in protokolla_map:
        protokolla_map[t] = sorted(protokolla_map[t])
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 1. Luodaan syöpätyypin valikko esilasketusta listasta
        valittu_syopatyyppi = st.selectbox("Syöpätyyppi", syopatyyppi_opts)
        
        # 2. Haetaan protokollat esilasketusta kartasta (O(1))
