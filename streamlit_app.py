...
def load_data():
    ...
    indikaatiot = set()
    protokollat_by_syopatyyppi = {"Kaikki": list(data.keys()), "Ei määritelty": []}

    for nimi, prot_data in data.items():
        tyypit = prot_data.get('syöpätyypit', [])
        if tyypit:
            # Käytetään settiä, jotta vältetään saman protokollan lisääminen useasti,
            # jos raw data sattuisi sisältämään saman syöpätyypin useaan kertaan
            for t in set(tyypit):
                indikaatiot.add(t)
    ...
    return data, syopatyyppi_opts, protokollat_by_syopatyyppi

YKSIKKO_OPTS_BASE = ("mg/m2", "mg/kg", "AUC", "mg")

try:
    # Always update Tietokanta.data with cached/loaded data
    Tietokanta.data, SYOPATYYPPI_OPTS, PROTOKOLLAT_BY_SYOPATYYPPI = load_data()
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 1. Luodaan syöpätyypin valikko välimuistitetuilla vaihtoehdoilla
        valittu_syopatyyppi = st.selectbox("Syöpätyyppi", SYOPATYYPPI_OPTS)
        
        # 2. Haetaan protokollat O(1) välimuistitetusta hakemistosta
