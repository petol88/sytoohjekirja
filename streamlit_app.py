...
def load_data():
    ...
    # Pre-calculate UI options to avoid redundant O(N) operations per Streamlit rerun
    indikaatiot = set()
...
# Initialize fallbacks in case of load failure
syopatyyppi_opts = ("Kaikki",)
protokolla_map = {"Kaikki": []}

try:
    # Always update Tietokanta.data with cached/loaded data
    # st.cache_data caches the returned tuple, avoiding recalculations on reruns
...
if view == "Laskuri":
    ...
    with col2:
        ...
        # 1-2. Valitaan syöpätyyppi valmiiksi lasketuista vaihtoehdoista
        valittu_syopatyyppi = st.selectbox("Syöpätyyppi", syopatyyppi_opts)
        
        # 3. Suodatetaan protokollat valitun syöpätyypin perusteella nopeasti sanakirjasta
