...
def load_data():
    ...
    indikaatiot = {
        t for prot in data.values()
        for t in (prot.get('syöpätyypit') or ["Ei määritelty"])
    }

    protokolla_map = {"Kaikki": list(data.keys())}
    for ind in indikaatiot:
        if ind == "Ei määritelty":
            protokolla_map[ind] = [nimi for nimi, p in data.items() if not p.get('syöpätyypit')]
        else:
            protokolla_map[ind] = [nimi for nimi, p in data.items() if ind in p.get('syöpätyypit', [])]
...
try:
    ...
    # Initialize with empty fallbacks to prevent UnboundLocalError during exception handling
    Tietokanta.data = {}
    SYOPATYYPPI_OPTS = ()
    PROTOKOLLA_MAP = {}
