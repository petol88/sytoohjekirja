...
def load_data():
    ...
    syopatyyppi_opts_set = set()
    protokolla_map = {"Kaikki": list(Tietokanta.data.keys()), "Ei määritelty": []}

    for nimi, pdata in Tietokanta.data.items():
        tyypit = pdata.get('syöpätyypit', [])
        if not tyypit:
            protokolla_map["Ei määritelty"].append(nimi)
            syopatyyppi_opts_set.add("Ei määritelty")
        else:
            for t in tyypit:
                syopatyyppi_opts_set.add(t)
                if t not in protokolla_map:
                    protokolla_map[t] = []
                protokolla_map[t].append(nimi)

    syopatyyppi_opts = ("Kaikki",) + tuple(sorted(syopatyyppi_opts_set))
    return Tietokanta.data, syopatyyppi_opts, protokolla_map

YKSIKKO_OPTS_BASE = ("mg/m2", "mg/kg", "AUC", "mg")

try:
    # Ensure fallbacks exist in case of exception to avoid UnboundLocalError
    _data, SYOPATYYPPI_OPTS, PROTOKOLLA_MAP = {}, ("Kaikki",), {"Kaikki": []}
