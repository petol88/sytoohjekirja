...
def load_data():
    ...
    # Pre-calculate UI widget options to avoid O(N) in render loop
    indikaatiot = {t for data in _data.values() if data.get('syöpätyypit') for t in data['syöpätyypit']}
    if any(not data.get('syöpätyypit') for data in _data.values()):
        indikaatiot.add("Ei määritelty")

    syopatyyppi_opts = ('Kaikki',) + tuple(sorted(indikaatiot))

    protokolla_map = {"Kaikki": ('',) + tuple(sorted(_data.keys()))}
    for t in indikaatiot:
        if t == "Ei määritelty":
            protokolla_map[t] = ('',) + tuple(sorted([nimi for nimi, data in _data.items() if not data.get('syöpätyypit')]))
        else:
            protokolla_map[t] = ('',) + tuple(sorted([nimi for nimi, data in _data.items() if t in data.get('syöpätyypit', [])]))
