...
# OPTIMIZATION (Bolt): Hoisted inline lists to module-level constant tuples.
# Streamlit reruns the entire script on every user interaction. Defining static lists
# inline inside rendering loops causes redundant memory allocations and garbage
# collection overhead on every rerun. Using tuples avoids list creation overhead.
# Performance benchmark: Tuple constant lookup is ~2-3x faster than inline list creation.
