...
def load_data():
    ...
    syopatyyppi_opts = ["Kaikki"] + sorted(list({
        t for p in data.values() if p.get('syöpätyypit') for t in p.get('syöpätyypit')
    }))

    protokolla_map = {"Kaikki": tuple(sorted(data.keys()))}

    ei_maaritellyt = tuple(sorted(nimi for nimi, p in data.items() if not p.get('syöpätyypit')))
    if ei_maaritellyt:
        syopatyyppi_opts.append("Ei määritelty")
        protokolla_map["Ei määritelty"] = ei_maaritellyt

    for tauti in syopatyyppi_opts:
        if tauti not in ("Kaikki", "Ei määritelty"):
            protokolla_map[tauti] = tuple(sorted(
                nimi for nimi, p in data.items() if tauti in p.get('syöpätyypit', [])
            ))

    return data, tuple(syopatyyppi_opts), protokolla_map
